			...
			<table id="branches" class="compact row-border cell-border" width="100%">
				...
				<tbody>
					...
					<tr>
					  ...
					  <td>{{$category->name}}</td>
					  <td>
                        <a href="{{route('admin.categories.edit', [$category->id])}}" title="تعديل" style="margin-left: 15px;"><i class="fa fa-edit fa-2x"></i></a>
                        <a data-bs-toggle="modal" data-bs-target="#categoriesModal{{$category->id}}" title="حذف" style="cursor: pointer;"><i class="fa fa-times fa-2x text-danger"></i></a>
					</td>
					</tr>
					<div class="modal fade" id="categoriesModal{{$category->id}}" tabindex="-1" aria-labelledby="categoriesModalLabel{{$category->id}}" aria-hidden="true">
						...
						<div class="modal-content">
							...
							<div class="modal-footer">
								...
								<a href="{{route('admin.categories.delete', $category->id)}}" class="btn btn-danger mx-1 font-1 save-image">{{trans('auth.delete')}}</a>
							</div>
						</div>
						</div>
					</div>
