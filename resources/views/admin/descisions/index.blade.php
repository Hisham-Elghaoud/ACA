...
<div class="col-12 px-0 py-2" style="background: #fff;">
	...
	<div class="row">
		<div class="col-1"></div>
		<div class="col-10">
			<a href="{{route('admin.descisions.create')}}"  class="btn btn-primary">إضافة</a>
		</div>
		<div class="col-1"></div>
	</div>
	<div class="row">
			...
			<table id="branches" class="compact row-border cell-border" width="100%">
				...
				<tbody>
					...
					<tr>
					  ...
					  <td>{{$report->decision_num}}</td>
					  <td>{{$report->name}}</td>
					  <td>{{$report->year}}</td>
					  <td>
                        <a href="{{route('admin.descisions.edit', [$report->id])}}" title="تعديل" style="margin-left: 15px;"><i class="fa fa-edit fa-2x"></i></a>
					...
					<div class="modal fade" id="reportModal{{$report->id}}" tabindex="-1" aria-labelledby="reportModalLabel{{$report->id}}" aria-hidden="true">
						...
						<div class="modal-content">
							...
							<div class="modal-footer">
								...
								<a href="{{route('admin.descisions.delete', $report->id)}}" class="btn btn-danger mx-1 font-1 save-image">{{trans('auth.delete')}}</a>
							</div>
						</div>
						</div>
					</div>
