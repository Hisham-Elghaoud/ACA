			...
			<table id="branches" class="compact row-border cell-border" width="100%">
				...
				<tbody>
					...
					<tr>
					  ...
					  <td>{{$report->legislation_num}}</td>
					  <td>{{$report->name}}</td>
					  <td>{{$report->year}}</td>
					  <td>
                        <a href="{{route('admin.legislation.edit', [$report->id])}}" title="تعديل" style="margin-left: 15px;"><i class="fa fa-edit fa-2x"></i></a>
					...
					<div class="modal fade" id="reportModal{{$report->id}}" tabindex="-1" aria-labelledby="reportModalLabel{{$report->id}}" aria-hidden="true">
						...
						<div class="modal-content">
							...
							<div class="modal-footer">
								...
								<a href="{{route('admin.legislation.delete', $report->id)}}" class="btn btn-danger mx-1 font-1 save-image">{{trans('auth.delete')}}</a>
							</div>
						</div>
						</div>
					</div>
