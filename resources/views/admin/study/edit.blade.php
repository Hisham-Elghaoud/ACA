...
<div class="row">
    <div class="col-1"></div>
    <div class="col-10">
        <form action="{{route('admin.study.update', [$report->id])}}" method="post" enctype="multipart/form-data">
            @csrf
            @method("PUT")
            <div class="form-group">
              <label>رقم التقرير</label>
              <input type="text" required name="study_num" value="{{$report->study_num}}" class="form-control">
            </div>
            <div class="form-group">
              <label>الإسم</label>
